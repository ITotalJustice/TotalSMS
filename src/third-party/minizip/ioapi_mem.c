...
voidpf ZCALLBACK fopen_mem_func(voidpf opaque, ZIP_UNUSED const char *filename, int mode)
{
    ...
    if (mode & ZLIB_FILEFUNC_MODE_CREATE)
    {
        if (mem->grow)
        {
            ...
            mem->data.w = (char *)malloc(mem->size);
        }
...
uint32_t ZCALLBACK fread_mem_func(ZIP_UNUSED voidpf opaque, voidpf stream, void *buf, uint32_t size)
{
    ...
    if (mem->flags == ourmemory_type_read) {
        memcpy(buf, mem->data.r + mem->cur_offset, size);
    }
    else {
        memcpy(buf, mem->data.w + mem->cur_offset, size);
    }
    mem->cur_offset += size;
...
uint32_t ZCALLBACK fwrite_mem_func(ZIP_UNUSED voidpf opaque, voidpf stream, const void *buf, uint32_t size)
{
    ...
    if (size > mem->size - mem->cur_offset)
    {
        if (mem->grow)
        {
            ...
            memcpy(newbase, mem->data.w, mem->size);
            free(mem->data.w);
            mem->data.w = newbase;
            mem->size = newmemsize;
        }
    ...
    memcpy(mem->data.w + mem->cur_offset, buf, size);
    mem->cur_offset += size;
...
static void fill_memory_filefunc_internal(zlib_filefunc_def *pzlib_filefunc_def, ourmemory_t *ourmem, enum ourmemory_type type)
{
...
{
    fill_memory_filefunc_internal(pzlib_filefunc_def, ourmem, ourmemory_type_write);
}

void fill_memory_filefunc_const(zlib_filefunc_def *pzlib_filefunc_def, ourmemory_t *ourmem)
{
    fill_memory_filefunc_internal(pzlib_filefunc_def, ourmem, ourmemory_type_read);
}
